...
export async function seedRankingEntry(
  rawNick: unknown,
  rawScore: unknown,
  ...
  if (process.env.NODE_ENV !== "development") {
    throw new Error("seedRankingEntry is only available in development");
  }
  ...
  const redis = getRedisClient();
  if (redis) {
    const key = getRankingKey(date, period);
    await redis.zadd(key, { score, member: nick });
    return { ok: true as const, nick, score, period, daysAgo, storage: "redis" as const };
  }

  let memKey: string;
  if (period === "weekly") memKey = `week:${getWarsawWeekKey(date)}`;
  else if (period === "monthly") memKey = `month:${getWarsawMonthKey(date)}`;
  else memKey = getWarsawDateKey(date);

  const map = memoryStore[memKey] ?? new Map<string, number>();
  memoryStore[memKey] = map;
  map.set(nick, score);

  return { ok: true as const, nick, score, period, daysAgo, storage: "memory" as const };
}

export function resetRankingStore() {
